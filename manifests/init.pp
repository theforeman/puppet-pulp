...
#                               type:boolean
#
#  $node_parent_host::          Hostname of the parent pulp node.
#                               Used only if $enable_child_node is set to true 
#
...
class pulp (
  ...
  $node_parent_host          = $pulp::params::node_parent_host,
  ...
  if $broker_manage {
    if $messaging_transport == 'qpid' {
      include ::qpid
      $broker_service = 'qpidd'
    } elsif $messaging_transport == 'rabbitmq' {
      include ::rabbitmq
  ...
  if $enable_child_node {
    ...
    if !defined(Class['pulp::consumer']) {
      class { 'pulp::consumer':
        host                 => $node_parent_host,
